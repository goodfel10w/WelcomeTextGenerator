...
server.registerTool("extract_from_pdf", {
...
server.registerTool("generate_onboarding_text", {
...
function extractDataFromText(text) {
    ...
    if (companyMatch)
        data.previousCompany = companyMatch[1].trim();
    return data;
}
// Hilfsfunktion: Willkommenstext generieren
function generateWelcomeText(data, focusAreas, tone) {
    ...
    if (tone === "professional") {
        ...
        if (focusAreas.experience && experience) {
            ...
            if (previousCompany)
                text += ` und war zuletzt bei ${previousCompany} tätig`;
            text += ". ";
        }
        ...
        if (focusAreas.experience && experience) {
            ...
            if (previousCompany)
                text += ` und kommt von ${previousCompany}`;
            text += ". ";
        }
        if (focusAreas.skills && skills.length > 0) {
        ...
        if (focusAreas.experience && experience) {
            ...
            if (previousCompany)
                text += ` und der Zeit bei ${previousCompany}`;
