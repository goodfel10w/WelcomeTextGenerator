import { introductionVariants, competenciesTemplate, funFactTemplate, experienceTemplate, closingVariants, } from "../templates/moduleTemplates.js";
/**
 * Generiert einen modularen Willkommenstext basierend auf dem Template-System
 */
export function generateModularWelcomeText(data, options = {}) {
    const { includeCompetencies = true, includeFunFact = false, includeExperience = true, introductionVariant = "variant1", closingVariant = "variant1", } = options;
    const modules = [];
    // Modul 1: Begrüßung & Einleitung (Pflicht)
    const introduction = introductionVariants[introductionVariant](data.name, data.position || "Mitarbeiter", "unserem Team" // Kann später aus data.team kommen
    );
    ...
    if (includeFunFact && data.interests.length > 0) {
        const funFact = funFactTemplate(data.name, data.interests[0], "Vielleicht erleben wir das bald gemeinsam im Team!");
    ...
    if (includeExperience && data.previousCompany && data.experience) {
        ...
        const experience = experienceTemplate(data.name, data.position || "Spezialist", data.previousCompany, responsibilities.length > 0 ? responsibilities : ["wichtige Projekte verantwortet"], additionalInfo);
